...
NTL.phys.data.Tuesday <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake") & year4 %in% c(1990:1999))
```
...
NTL.phys.data.TuesdayFilter <- 
  NTL.phys.data %>% 
  filter(lakename == "Tuesday Lake") %>%
  #select(lakename, year4:sampledate) 
  filter(year4 %in% c(1990:1999)) %>% 
  filter(month(sampledate)==7)

#mutate(month = (sampledate)) 
#month function from lubridate 
