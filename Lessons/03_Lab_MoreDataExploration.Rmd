...
# Lists ----  similar to calling up certain columns
...
```{r}
high <- c(57,40,41,44,37,41,51,52,52,60)
low <- c(30,18,26,26,18,26,26,28,40,44)
```
...
high_celsius <- celsius(high)
high_celsius
low_celsius <- celsius(low)
low_celsius
```
...
```{r}
temps_df <- data.frame(high,low,high_celsius,low_celsius)

#temps_df <- data.frame(high = "High (F)",low = "Low (F)",high_celsius = "High (C)",low_celsius = "Low (C)")
```
